...
import type { ArchitectDashboardResponseDto, RegisterResponseDto } from "@/generated/api";
import { clearAuthSession, isOrganizationAdmin } from "@/lib/auth-session";
import { loadArchitectDashboard, type LoadArchitectDashboardResult } from "@/lib/dashboard";
import { architectDashboardPath } from "@/lib/dashboard-routing";
import { useRequiredAuthSession } from "@/lib/use-auth-session";
import { ArchitectDashboard } from "./architect-dashboard";

export type ArchitectDashboardLoader = (
  accessToken: string,
  siteId: string | null,
) => Promise<LoadArchitectDashboardResult>;

interface ArchitectDashboardShellProps {
  readonly dashboardLoader?: ArchitectDashboardLoader;
}
...
interface ArchitectDashboardAuthenticatedContentProps {
  readonly dashboardLoader: ArchitectDashboardLoader;
  readonly session: RegisterResponseDto;
}

interface UseArchitectDashboardParams {
...
export function ArchitectDashboardShell({
  dashboardLoader = loadArchitectDashboard,
}: ArchitectDashboardShellProps) {
  ...
  return (
    <main className="min-h-screen bg-background">
      <AppHeader
        ...
        dashboardHref={architectDashboardPath}
        showSettingsLink={isOrganizationAdmin(session)}
      />

      <section className="container py-8">
        <Suspense
          fallback={<DashboardLoadingCard message="Chargement du dashboard architecte..." />}
        >
          <ArchitectDashboardAuthenticatedContent
...
function ArchitectDashboardAuthenticatedContent({
  dashboardLoader,
  session,
}: ArchitectDashboardAuthenticatedContentProps) {
  const searchParams = useSearchParams();
  const siteId = searchParams.get("siteId");
  const { loadState, refreshDashboard } = useArchitectDashboard({
...
function useArchitectDashboard({
  accessToken,
  dashboardLoader,
  siteId,
}: UseArchitectDashboardParams) {
  ...
  useEffect(() => {
    ...
    // TODOOOOO : à remplacer avec la vraie donnée temps réel BIM/IA quand les modules seront prets
...
function DashboardBody({
  ...
  if (loadState.status === "loading") {
    return <DashboardLoadingCard message="Chargement des projets BIM accessibles..." />;
  }
    ...
    <Card>
      <CardContent className="flex flex-col gap-4 p-6 md:flex-row md:items-center md:justify-between">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold">Dashboard architecte indisponible</h1>
...
function applyDashboardLoadResult(
  result: LoadArchitectDashboardResult,
  setLoadState: Dispatch<SetStateAction<DashboardLoadState>>,
